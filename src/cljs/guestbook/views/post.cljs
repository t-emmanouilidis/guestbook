    ...
    (if (= post-id root_id)
      (let [parent-id (:id (second messages))]
        (if (= parent-id post-id)
          (update-in db [::post :reply-count] inc)
          (update db ::posts
                  #(if (contains? % parent-id)
                     (update-in % [parent-id :reply-count] inc)
                     %))))
      db)))
...
(def post-controllers
  [{:parameters {:path [:post-id]}
    :start      (fn [{{:keys [post-id]} :path}]
                  (rf/dispatch [:ws/set-message-add-handler [::add-message post-id]])
                  (rf/dispatch [:post/fetch post-id])
                  (rf/dispatch [:post/fetch-replies post-id]))
    :stop       (fn [_]
                  (rf/dispatch [:ws/set-message-add-handler nil])
                  (rf/dispatch [::collapse-all])
                  (rf/dispatch [:post/clear])
                  (rf/dispatch [:post/clear-replies]))}
   {:parameters {:query [:reply-id]}
    :start      (fn [{{:keys [reply-id]} :query}]
                  (when reply-id
                    (rf/dispatch [::set-scroll-to reply-id])
                    (rf/dispatch [::fetch-parents reply-id])))
    :stop       (fn [_]
    ...
    {:component-did-mount
     (fn [this]
       (when @(rf/subscribe [::scroll? post-id])
         (rf/dispatch [::set-scroll-to nil])
         (.scrollIntoView (dom/dom-node this)))) :reagent-render
     (fn [_] [msg/message
              @(rf/subscribe [::reply post-id]) {:include-link? false}])}))
...
(defn expand-control [post-id]
  (let [expanded? @(rf/subscribe [::post-expanded? post-id])
    ...
    [:div.field.has-addons
     ...
     (when expanded?
       [:p.control>button.button
        {:on-click #(rf/dispatch [:post/fetch-replies post-id])
         :disabled (= status :loading)}
        (if loaded?
          "♻️"
