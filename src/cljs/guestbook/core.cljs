...
(rf/reg-fx
  ...
  (fn [{:keys [message timeout callback-event] :or {timeout 30000}}]
    (if callback-event
      (ws/send! message timeout (fn [r] (rf/dispatch (conj callback-event r))))
       ...
       (merge attrs
              {:type      :text
               ...
               :on-change (fn [eventObj]
                            (reset! draft (.-value (.-target eventObj))))
               :value     @value})])))
...
(defn message-list []
  [:ul.messages
   (for [{:keys [timestamp message name author]} @(rf/subscribe [:messages/list])]
     ^{:key timestamp}
     [:li
